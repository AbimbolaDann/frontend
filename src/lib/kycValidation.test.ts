...
describe('security validation edge cases', () => {
  ...
  it('rejects malicious address fields', () => {
    ...
    expect(validateAddress({ ...valid, state: '<svg/onload=alert(1)>' }).state).toBe('State contains invalid characters');
    expect(validateAddress({ ...valid, zip: "1 UNION SELECT" }).zip).toBe('ZIP code contains invalid characters');
